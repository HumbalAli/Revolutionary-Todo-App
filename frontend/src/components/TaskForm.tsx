      ...
      <form onSubmit={handleSubmit} className="flex flex-col space-y-8">
        <div className="space-y-4">
          <input
            ...
            className="w-full bg-transparent border-none focus:ring-0 text-2xl font-bold placeholder:text-gray-700 transition-all tracking-tight"
            disabled={loading}
          />

          <div className="h-px bg-gradient-to-r from-blue-500/50 via-white/5 to-transparent w-full"></div>

          <textarea
            value={description}
            onChange={(e) => setDescription(e.target.value)}
            placeholder="Detailed metadata (Optional)"
            className="w-full bg-transparent border-none focus:ring-0 text-sm text-gray-400 placeholder:text-gray-800 font-light resize-none h-12"
            disabled={loading}
          />
        </div>
